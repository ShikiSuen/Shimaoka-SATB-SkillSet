...
`SKILL.md` 作为入口档，提供完整的术语体系、8 步骤写作流程与概念速查；`references/` 下 13 个子文件按主题细分，Agent 可按需检索。`_SKILL-Amalgamated.md` 则是全量合并档，方便一次性载入。
...
├── Makefile                           # 构建脚本
├── VALUEADD/Reviews/                  # 各 LLM 对体系的评测与见解
...
│   └── ...（共 9 篇）
└── shimaoka-harmony/                  # 知识库主体
    ├── SKILL.md                       # 入口：核心哲学、术语、写作步骤、概念速查
    ├── _SKILL-Amalgamated.md          # 全量合并档（14 个 .md 合一）
    └── references/                    # 细项参考文件（14 个）
...
全部 14 个 markdown 档案（SKILL.md + references/*.md）的合并版。适用于需要一次性载入完整知识库的场景（如上下文窗口充裕或需要离线参考）。档名以 `_` 开头表示它是衍生档，由 `make amalgamated` 生成。

### `references/`
13 个细项参考文件，每个聚焦一个主题。Agent 按需单独检索对应文件，比加载整份合并档更节省 token。
